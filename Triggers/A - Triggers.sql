...
AS
    -- Body of Trigger
    IF @@ROWCOUNT > 0  -- It's good to see if any rows were changed
       AND
       EXISTS (SELECT * FROM Activity 
...
VALUES (200122100, 'CIPS'), -- Peter Codd
       ...
       (200495500, 'CIPS') -- Robert Smith, his 4th club
...
UPDATE Course SET CourseCost = CourseCost * 1.20
...
SELECT * FROM BalanceOwingLog -- To see whats in there after a hack attempt
UPDATE Student SET BalanceOwing = 10000 -- He graduated and doesn't want competition
SELECT * FROM BalanceOwingLog -- To see whats in there after a hack attempt